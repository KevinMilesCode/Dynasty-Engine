...
> This file is the real thing in the meantime — same content, same process.
> Once the repo goes public (free tier includes Wikis for public repos) or
> moves to a paid plan, this same content mirrors over to a real Wiki
> automatically (`wiki-sync.yml`) and this note goes away. **Everything below
> is deliberately kept to this one page** while the project's still a small
> preview — split it into multiple pages once there's enough content (and a
> real audience) to justify the navigation overhead.
...
- **Question?** [Ask in Discussions → Q&A](https://github.com/KevinMilesCode/Dynasty-Engine/discussions/categories/q-a) —
  once there's enough Q&A traffic to be worth a dedicated FAQ page, answered
  questions get folded in by hand (see `MAINTAINING.md`), not by an
  automated bot.
